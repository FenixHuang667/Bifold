...
PUBLIC char *pair2structure(int *pair);
PUBLIC int *structure2pair(char *s);
...
PUBLIC char *pair2structure(int *pair)
{
	...
	res=(char *) calloc (l+2, sizeof(char));
...
void randomseq(int n, char *res)
{
	...
	for (i=0;i<n;i++) {
	  seed=(1-drand48())*4;
	  if (seed<=1) res[i]='A';
	  else if (seed>1 && seed <=2) res[i]='U';
	  else if (seed>2 && seed <=3) res[i]='C';
	  else if (seed>3 && seed <=4) res[i]='G';
	}
	res[n]=0;
}
...
char randomnucleotide(void)
{
    ...
    seed=(1-drand48())*4;
    if (seed<=1) return 'A';
    else if (seed>1 && seed <=2) return 'U';
    else if (seed>2 && seed <=3) return 'C';
    else if (seed>3 && seed <=4) return 'G';
    return '_';
}
...
int idetical_struc(int *pa, int *pb)
{
    int i;
...
int *read_patten(char *pat)
{
    int i, l= strlen(pat), s = 0;
    int *p, *stack;
    
    p = (int *) calloc (l+5, sizeof(int));
    stack = (int *) calloc (l+5, sizeof(int));
    
    p[0] = l;
    for (i=0; i<l; i++)
    {
        if (pat[i] == '.') p[i+1] = 0; //unpaired
        else if (pat[i] == 'x' || pat[i] == 'X') p[i+1] = -2; // no constrain
        else if (pat[i] == '|') p[i+1] = -1; //paired
        else if (pat[i] == '(') {
            stack[s] = i;
            s++;
        } else if (pat[i] == ')') {
...
int * seq2code(char *seq)
{
    int i, l = strlen(seq);
    int *pat;
    
    pat = (int *) calloc (l+5, sizeof(int));
    pat[0] = l;
    for (i=0; i<l; i++)
        switch (seq[i]) {
            case 'A':
...
char code2nucleotide (int a)
{
    if (a==1) return 'A';
    else if (a==2) return 'C';
    else if (a==3) return 'G';
    else if (a==4) return 'U';
    else return '_';
}

int nucleotide2code (char a)
{
    if (a=='A') return 1;
