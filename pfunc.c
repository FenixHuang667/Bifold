...
void pfunc_initial (int n)
{
    ...
    Pc     = (float *) calloc(((n*(n+1))/2+2), sizeof(float));
    Pf5    = (float *) calloc(((n*(n+1))/2+2), sizeof(float));
    Pf51   = (float *) calloc(((n*(n+1))/2+2), sizeof(float));
    PfML   = (float *) calloc(((n*(n+1))/2+2), sizeof(float));
    PfML1  = (float *) calloc(((n*(n+1))/2+2), sizeof(float));
...
void structural_prob (char *seq, int *pat)
{
    int i,j, p, q, k1, r;
    int *s;
    int length,type1,type2,G,type, available;
    ...
    Pf5[indx2[length]+1] = 1;
    
    s = encode_seq(seq);
    
    for (i=1; i<=length-TURN; i++) {
        for (j=length; j>=i+TURN; j--) {
            if (Qf5[indx2[j]+i]!=0) {
                Pf51[indx2[j]+i] += (Qf51[indx2[j]+i]/Qf5[indx2[j]+i]) * Pf5[indx2[j]+i];
                for (k1=i;k1<j;k1++) {
                    temp_result = (Qf51[indx2[k1]+i]*Qf5[indx2[j]+k1+1])/Qf5[indx2[j]+i];
                    Pf51[indx2[k1]+i] += temp_result * Pf5[indx2[j]+i];
                    Pf5[indx2[j]+k1+1] += temp_result * Pf5[indx2[j]+i];
                }
            }
            //compute Pf51
            ...
            if (Qf51[indx2[j]+i]!=0) {
                for (k1=i;k1<j;k1++) {
                    ...
                    if (type>2) G=P->TerminalAU; else G=0;
                    if (available) {
                        temp_result = (Qc[indx2[j]+k1]*EXP(G))/Qf51[indx2[j]+i];
                        Pc[indx2[j]+k1] += temp_result * Pf51[indx2[j]+i];
                        if (k1>i) Pf5[indx2[k1-1]+i] += temp_result * Pf51[indx2[j]+i];
                    }
                }
            }
            //compute Qf51[i,j]
            
            if (QfML1[indx2[j]+i]!=0) {
                for (k1=i;k1<j;k1++) {
                    available = 1;
                    type=BP_pair[s[k1]][s[j]];
                    if (!type) available = 0;
                    ...
                    if (available) {
                        G=P->MLintern[type]+(k1-i)*P->MLbase;
                        temp_result = Qc[indx2[j]+k1]*EXP(G) / QfML1[indx2[j]+i];
                        Pc[indx2[j]+k1] += temp_result * PfML1[indx2[j]+i];
                    }
                }
            }
            //compute QfML1[i,j]
            ...
            if (QfML[indx2[j]+i]!=0) {
                for (k1=i;k1<j;k1++) {
                    ...
                    if (available) {
                        G=(j-k1)*P->MLbase;
                        temp_result = QfML1[indx2[k1]+i]*(QfML[indx2[j]+k1+1])*EXP(G) / QfML[indx2[j]+i];
                        PfML1[indx2[k1]+i] += temp_result * PfML[indx2[j]+i];
                        PfML[indx2[j]+k1+1] += temp_result * PfML[indx2[j]+i];
                    }
                }
            }
                ...
                if (type && check_available(i, pat, j) && check_available(j, pat, i)) {
                    for (p = i+1; p <= j-2-TURN; p++) {
                        for (q = p+TURN; q < j; q++) {
                            ...
                            G = LoopEnergy(p-i-1, j-q-1, type, type2, s[i+1], s[j-1], s[p-1], s[q+1]);
                            temp_result = EXP(G)*Qc[indx2[q]+p]/Qc[indx2[j]+i];
                            Pc[indx2[q]+p] += temp_result * Pc[indx2[j]+i];
                            //contribution from int, bulge, helix
                        }
                    }
                    for (k1=i+1;k1<j-1;k1++) {
                        G=P->MLintern[type]+P->MLclosing;
                        temp_result = QfML1[indx2[k1]+i+1]*QfML[indx2[j-1]+k1+1]*EXP(G) / Qc[indx2[j]+i];
                        PfML1[indx2[k1]+i+1] += temp_result * Pc[indx2[j]+i];
                        PfML[indx2[j-1]+k1+1] += temp_result * Pc[indx2[j]+i];
                        //contribution from mul
                    }
                } else {
                    Pc[indx2[j]+i]=0;
...
float ** pair_region (char *seq, int *pat)
{
    int i, length=strlen(seq), j, l, h;
    float **Fup, **Fdown, **score, sp=1, sn=0.9;
    
    Fup = (float **) calloc (length+5, sizeof(float *));
    Fdown = (float **) calloc (length+5, sizeof(float *));
    score = (float **) calloc (length+5, sizeof(float *));
    for (i=0; i<=length; i++) {
        Fup[i] = (float *) calloc (length+5, sizeof (float));
        Fdown[i] = (float *) calloc (length+5, sizeof (float));
        score[i] = (float *) calloc (length+5, sizeof (float));
        for (j=0;j<=length;j++) {
            Fup[i][j] = 0;
            Fdown[i][j] = 0;
            score[i][j] = 0;
        ...
        for (l=j+1; l<=length; l++) {
            ...
            for (h=j; h<l; h++) {
                for (i=h+1; i<=length; i++) {
                    Fdown[j][l] += Pc[indx2[i]+h];
                }
            }
            Fdown[j][l] /= (l-j);
            
            score[j][l] = (sp*Fup[j][l]+sn*Fdown[j][l]) * (sp*Fdown[j][l]+sn*Fup[j][l]);
            if (l-j==5) printf("[%d, %d]: %f %f %f\n", j, l, Fup[j][l], Fdown[j][l], score[j][l]);
        }
    }
    return score;
}
