...
patten *patten_detector(char *seq, int i, int j, int frequency, patten *List)
{
    ...
    str = (char *) calloc ((j-i+10), sizeof(char));
    for (r=0; r<j-i+1; r++) {
        str[r] = seq[i+r];
    }
    str[j-i+1] = 0;
...
void seq_analysis (FILE *input)
{
    char name[100], seq_list[5000][300];
    int freA[300], freU[300], freG[300], freC[300];
    int i, j, num, length;
    fscanf(input, "%s\n", name);
    i=0;
    while (!feof(input)) {
    ...
    length = strlen(seq_list[0]);
    for (i=0; i<num; i++) {
        for (j=0; j<length; j++) {
            if (seq_list[i][j] == 'A') freA[j]++;
            else if (seq_list[i][j] == 'C') freC[j]++;
            else if (seq_list[i][j] == 'G') freG[j]++;
            else if (seq_list[i][j] == 'U') freU[j]++;
        }
    }
    
    for (i=0; i<length; i++) {
        printf("A: %d C:%d G: %d U: %d\n", freA[i], freC[i], freG[i], freU[i]);
    }
    printf("%d\n", length); 
    for (i=0; i<num; i++) {
        for (j=i+1; j<num; j++) {
            printf("%d ", hamming_distance(seq_list[i], seq_list[j]));
        }
        printf("\n");
    }
}
